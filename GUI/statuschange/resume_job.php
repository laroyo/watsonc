...
if($response_array["state"] == "paused") {
	$resume_job = "curl \"https://api.crowdflower.com/v1/jobs/$job_id/resume.json?key=$api_key\"";
	$resume_response = exec($resume_job);
	$resume_response_array = json_decode($resume_response);

	if (array_key_exists("success", $resume_response_array)) {
		echo "The job was resumed";
    //  update the database
		$updateDB = mysql_query("Update history_table Set status = '$status' Where job_id = '$job_id' ") or mysql_error();
	}
}
else {
	echo "The job cannot be resumed";
}
?>
